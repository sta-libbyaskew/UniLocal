...
public class ModelContainer {
    ...
    public void instancePreFlattening2(boolean propagate) {
        ...
        if(!propagate && (CmdFlags.make_short_tab==3 || CmdFlags.make_short_tab==4)) {
            //  Normalise to remove any duplicate constraints that would trigger identical scopes heuristic.
            m.transform(tn);
        ...
        if(CmdFlags.getSattrans() && !propagate) {
            if(CmdFlags.getSatDelFunc()) {
                //  Remove decision variables where they can be replaced with a linear 
                //  mapping of another decision variable.
                TransformDeleteFuncVars tdfv=new TransformDeleteFuncVars(m);
                m.transform(tdfv);
            }
    ...
    public void squashDomains() {
        ...
        if(CmdFlags.getAmoDetect() && CmdFlags.amo_detect_noamoenc) {
            // Throw away all PB constraints.
            TransformPBToNil tpb=new TransformPBToNil(m);
            m.transform(tpb);
        }
    ...
    public void instanceFlattening(boolean propagate) {
        ...
        //  Late pass of tabulation
        if(CmdFlags.tabulate_late && !propagate) {
            // Hackety hack -- change the tabulation setting
            int tmp=CmdFlags.make_short_tab;
            CmdFlags.make_short_tab=4;  // short, heuristics.
            TransformMakeTable tmt=new TransformMakeTable(m, propagate);
    ...
    public void makeTableScopes() {
        ...
        //  Manipulate flag to run SR as if -make-tables were there.
    ...
    private void decomposeSatEncodingFlat() {
        ...
        if(CmdFlags.getSatPBMDD() || CmdFlags.getSatSumMDD() || CmdFlags.getSatPBGPW() || CmdFlags.getSatSumGPW()
            || CmdFlags.getSatPBSWC() || CmdFlags.getSatSumSWC() || CmdFlags.getSatPBGGT() || CmdFlags.getSatSumGGT()) {
            //  Catch AMO-PB constraints so they can be encoded using the MDD/GPW/SWC/GGT encodings
