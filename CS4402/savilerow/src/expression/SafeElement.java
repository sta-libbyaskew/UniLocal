...
// Element(matrix expression, index expression) is a function to the result.
    ...
    @Override public ASTNode simplify() {
        ...
        if(mat instanceof CompoundMatrix) {
            ...
            if(a.upper<numelements-1) {
                ...
                int newsize=((int)a.upper)+1;
                if(newsize<0) {
                    newsize=0;
                }
                
                ArrayList<ASTNode> newelements=new ArrayList<ASTNode>(newsize);
                if(mat==getChild(0)) {
                    mat.detachChildren();
                }
