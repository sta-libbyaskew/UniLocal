...
public class TransformQuantifiedExpression extends TreeTransformerTopdown
{
    ...
    protected NodeReplacement processNode(ASTNode curnode)
    {
        if(curnode instanceof Quantifier) {
            ...
            ArrayList<ASTNode> cm=unrollComprehension(curnode, curnode.getChild(0), curnode.getChild(1), curnode.getChild(2), 0);
    ...
    ArrayList<ASTNode> unrollComprehension(ASTNode originalexp, ASTNode innerexp, ASTNode quants, ASTNode condition, int quantidx) {
        TransformSimplify ts=new TransformSimplify();
        ...
        if(quantidx<quants.numChildren()) {
            if(quants.getChild(quantidx).getChild(1) instanceof MatrixDomain) {
                ...
                Pair<ArrayList<ASTNode>, ReplaceASTNode> p=enumerateMatrixDomain(quants.getChild(quantidx));
                ...
                return unrollComprehension(contextCM, iexp, iquant, icond, quantidx);  // Stay at same index. 
            }
            ...
            for(int i=0; i<vals.size(); i++) {
                for(long val=vals.get(i).lower; val<=vals.get(i).upper; val++) {
                    ...
                    cm.addAll(unrollComprehension(originalexp, iexp, iquant, icond, quantidx+1));
                }
            }
            return cm;
        }
    ...
    public Pair<ArrayList<ASTNode>, ReplaceASTNode> enumerateMatrixDomain(ASTNode compquantifier) {
