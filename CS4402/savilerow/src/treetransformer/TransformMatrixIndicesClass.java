    ...
    public TransformMatrixIndicesClass(int new_lower_bound, Model mod, String mn)
    {
        ...
        for(ASTNode indexdom : indices)
        {
            ...
            if(indexdom.getCategory() ==ASTNode.Constant) {
                ...
                ArrayList<Intpair> idxintervals=indexdom.getIntervalSet();
                if(idxintervals.size()>1) {
                    // There are holes in the index domain.
                    
                    ArrayList<Long> idxvals=new ArrayList<Long>();
                    for(int i=0; i<idxintervals.size(); i++) {
                        for(long val=idxintervals.get(i).lower; val<=idxintervals.get(i).upper; val++) {
                            idxvals.add(val);
                        }
                    }
                    ...
                    long lowervalue=idxintervals.get(0).lower;
                    long uppervalue=idxintervals.get(idxintervals.size()-1).upper;
